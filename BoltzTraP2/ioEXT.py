...
from BoltzTraP2 import plotpng
...
def _detect_vasp_interpolated_velocities(xml_tree):
    ...
    """
    candidates = ("./calculation/eigenvelocities[@comment=\"interpolated\"]/",
                  "./calculation/eigenvalues[@comment=\"interpolated_ibz\"]"
                  "/electronvelocities")
    for i in candidates:
...
def _parse_vasp_magmom(xml_tree):
    ...
    a vasprun.xml file.
    """
    xml_spin = xml_tree.find("./parameters"
    ...
    magmom = np.array(
        _parse_vasp_array(xml_spin.find("./v[@name=\"MAGMOM\"]").text.strip()))
    saxis = np.array(
        _parse_vasp_array(xml_spin.find("./v[@name=\"SAXIS\"]").text.strip()))
    if noncollinear:
        ...
        magmom[:, 0] = (np.cos(beta) * np.cos(alpha) * magmom0[:, 0] -
                        np.sin(alpha) * magmom0[:, 1] +
                        np.sin(beta) * np.cos(alpha) * magmom0[:, 2])
        magmom[:, 1] = (np.cos(beta) * np.sin(alpha) * magmom0[:, 0] +
                        np.cos(alpha) * magmom0[:, 1] +
                        np.sin(beta) * np.sin(alpha) * magmom0[:, 2])
        magmom[:, 2] = (-np.sin(beta) * magmom0[:, 0] +
                        np.cos(beta) * magmom0[:, 2])
    elif ispin == 1:
...
def _parse_vasp_kinter(xml_tree):
    ...
    for e in xml_incar:
        attribs = e.attrib
        if ("type" in attribs and attribs["type"] == "int"
...
def _parse_vasp_lvel(xml_tree):
    ...
    for e in xml_incar:
        attribs = e.attrib
        if ("type" in attribs and attribs["type"] == "logical"
...
def _get_vasp_kpoints_path(xml_tree):
    ...
    candidates = ("./calculation/eigenvalues[@comment=\"interpolated\"]/"
...
def parse_vasprunxml(filename):
    ...
    if _detect_vasp_broken_interpolation(xml_tree):
        raise ValueError("the interpolated band data in vasprun.xml is "
    ...
    nruter = dict(fermi=_parse_vasp_fermi(xml_tree),
    ...
    if toreduce:
        subset = calc_reciprocal_iksubset(nruter["atoms"], nruter["magmom"],
                                          nruter["kpoints"])
...
def save_traceEXT(filename,
    ...
    #changed/added by Yi Wang, 09/24/2020
    natoms = data.get_natoms()
    vuccm3 = data.atoms.get_volume() * 1e-24  # in cm^3
    #efermi = data.get_fermi()
    #print(efermi)

    headerfmt = "#{:>11s} {:>9s}" + " ".join(21 * ["{:>25s}"])
    ...
    header = [
        ...
        "L0_h", "L0_e", "M_h", "M_e", "N_h", "N_e", "deltaE(eV)", "tau(s)", "tau_1(s)"
    ]
    ...
    #changed by Yi Wang, 09/24/2020
    rowfmt = "{:>12.8g} {:>9g}" + " ".join(21 * ["{:>25.12g}"])
    with open(filename, "w") as f:
        ...
        for imu in range(nmu):
            for iT, T in enumerate(Tr):
                ...
                # formula.
                # changed into a quantity per mole of atom by Yi Wang in 09/24/2020
                ocv = cv[iT, imu] * AVOGADRO / natoms #nformulas
                ...
                #changed by Yi Wang 09/24/2020
                ocv_x = cv_x[iT, imu] * AVOGADRO / natoms #nformulas
                oS_x = S_x[iT, imu]
                oN_x = N_x[iT, imu]/vuccm3
                c_n = L_x[iT, imu]*qe_SI*qe_SI
                dE = T*np.sqrt(c_n)
                tau = hbar_SI/dE/2
                tau_1 = hbar_SI/qe_SI/deltaE[iT, imu]/2
                print(rowfmt.format(
                    (mu-efermi)/Volt, T, N[iT, imu] + data.nelect, sdos[iT, imu],
                    seebeck[iT, imu].trace() / 3.,
                    cond[iT, imu].trace() * conductivity_factor / 3., ohall,
                    kappa[iT, imu].trace() * conductivity_factor / 3., ocv,
                    magsus, ocv_x, oS_x, oN_x, L_x[iT, imu],
                    L0_h[iT, imu].trace() * conductivity_factor / 3.,
                    L0_e[iT, imu].trace() * conductivity_factor / 3.,
                    M_h[iT, imu].trace() * conductivity_factor / 3.,
                    M_e[iT, imu].trace() * conductivity_factor / 3.,
                    N_h[iT, imu]/vuccm3,
                    N_e[iT, imu]/vuccm3, deltaE[iT, imu], tau, tau_1),
                      file=f)

    plotpng.plotpng(filename, data, metadata=scattering_model)

def save_condtens(filename,
                  data,
                  Tr,
                  mur,
                  N,
...
def save_halltens(filename, data, Tr, mur, N, hall):
    ...
    with open(filename, "w") as f:
        ...
        for imu in range(nmu):
            for iT, T in enumerate(Tr):
                mu = mur[iT, imu]
                print(rowfmt.format(mu * 2., T, N[iT, imu] + data.nelect,
...
class AIMSReader:
    ...
    def read_AIMS_energies(self, filename="Final_KS_eigenvalues.dat"):
        ...
        nspins = (1 if (self.parameters["spin"] == None) else 2)
        if (self.parameters["SOC"] == False) and (nspins == 1):
            dosweight = 2.0
        else:
            dosweight = 1.0
        ...
        with open(filename, "r") as file:
            content = [
                ...
                if "k-point" not in line and "#" not in line
                and "occupation number (dn), eigenvalue (dn)" not in line
            ]
            content = [
                list(group) for k, group in groupby(content, lambda x: x == [])
                if not k
            ]  # this splits by empty lines
            if (self.parameters["SOC"] == False) and (nspins == 2):
                spinup = np.array(
                    content, dtype=float)[:, :, 2] * eV  # (nkpoints, bands)
                spindown = np.array(
                    content, dtype=float)[:, :, 4] * eV  # (nkpoints, bands)
            else:
                content = np.array(
                    content, dtype=float)[:, :, 2] * eV  # (nkpoints, bands)
        ...
        with BoltzTraP2.misc.dir_context(dirname):
            ...
            if len(filenames) > 1:
                ...
                for outfile in filenames:
                    check = os.popen("tail -n 10 {}".format(
                        os.path.join(dirname, outfile))).read()
