...
class VASPLoader:
    ...
    def __init__(self, directory):
        ...
        with BoltzTraP2.misc.dir_context(directory):            
            ...
            elif os.path.isfile("vasprun.xml.gz"):
                import gzip
                vasprunxml = gzip.open("vasprun.xml.gz","r")
            else:
            ...
            if self.ebands.shape[0] > 1 or (r["magmom"] is not None
                                            and r["magmom"].shape[1] == 3):
                self.dosweight = 1.0
            else:
                self.dosweight = 2.0
...
class ABINITLoader:
    ...
    def __init__(self, directory):
        ...
        with BoltzTraP2.misc.dir_context(directory):
            ...
            self.ebands = self.ebands.reshape(-1, self.ebands.shape[-1])
            self.nelect = r["nelect"]
            # TODO: for this option need to read other 1WFx files with velocity matrix elements
            if "v" in r:
    ...
    with BoltzTraP2.misc.dir_context(dirname):
        ...
        if len(filenames) > 1:
            logging.warning(
                ...
                " - using the first one")
    # use the first one we find, TODO eventually add options to choose the GSR file from a specific dataset
...
class DFTData:
    ...
    def __init__(self, directory, derivatives=False, *args, **kwargs):
        ...
        BoltzTraP2.misc.info("successfully loaded a {} calculation".format(
            self.source))
    ...
    def get_natoms(self):
        """Return the number of atoms in the full cell.
        """
        return sum(collections.Counter(self.atoms.get_chemical_symbols()).values())
