...
def save_trace(filename,
    ...
    volume = data.get_volume()*(0.529177)**3
    #print(volume)
    #changed by Yi Wang, 09/24/2020
    if cv_x is not None:
        headerfmt = "#{:>11s} {:>9s}" + " ".join(11 * ["{:>25s}"])
    else:
        headerfmt = "#{:>9s} {:>9s}" + " ".join(8 * ["{:>25s}"])
    header = [
        "Ef[Ry]", "T[K]", "N[e/uc]", "DOS(ef)[1/(Ha*uc)]", "S[V/K]",
        "sigma/tau0[1/(ohm*m*s)]", "RH[m**3/C]", "kappae/tau0[W/(m*K*s)]",
        "cv[J/(mole-atom*K)]", "chi[m**3/mol]"
    ]
    #added by Yi Wang 09/24/2020
    if cv_x is not None: header.extend(["cv_x[J/(mole-atom*K)]", "S_x(V/K)", "N_x(e/cm^3)"])
    ...
    #changed by Yi Wang, 09/24/2020
    if cv_x is not None:
        rowfmt = "{:>12.8g} {:>9g}" + " ".join(11 * ["{:>25g}"])
    else:
        rowfmt = "{:>12.8g} {:>9g}" + " ".join(8 * ["{:>25g}"])
    with open(filename, "w") as f:
        """
        import sys
        print(header,file=sys.stderr)
        print(headerfmt,file=sys.stderr)
        """
        ...
        for imu in range(nmu):
            for iT, T in enumerate(Tr):
                ...
                if cv_x is None:
                    print(rowfmt.format(
                        mu * 2., T, N[iT, imu] + data.nelect, sdos[iT, imu],
                    ...
                    oS_x = S_x[iT, imu]
                    oN_x = N_x[iT, imu]/volume*1e24
                    print(rowfmt.format(
                        mu * 2., T, N[iT, imu] + data.nelect, sdos[iT, imu],
                        seebeck[iT, imu].trace() / 3.,
                        cond[iT, imu].trace() * conductivity_factor / 3., ohall,
                        kappa[iT, imu].trace() * conductivity_factor / 3., ocv,
                        magsus, ocv_x, oS_x, oN_x),
                          file=f)
