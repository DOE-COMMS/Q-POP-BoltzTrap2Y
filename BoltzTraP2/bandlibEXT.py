...
from BoltzTraP2.fd import _FD_XMAX_GAP, _FD_XMAX
...
def lambda_to_tau(vvband, lambdas):
    ...
    if (not isinstance(vvband, np.ndarray) or vvband.ndim != 4
...
def calc_N(epsilon, dos, mu, T, dosweight=2.):
    ...
    return -trapz(dosweight * (dos * occ), epsilon)
...
def solve_for_mu(epsilon,
    ...
    if 1==1 and T!=0:
        ...
        mu = brentq(gfind, -0.5, 0.5, args=(epsilon, dosweight*dos, N0, Beta, trapz), maxiter=10000)
        return mu
    ...
    """
    delta = -cumtrapz(dosweight * dos, epsilon) + N0
    ...
    if refine:
        ...
        if center:
            ...
            if hpos != lpos:
                ...
                try:
                    result = sp.optimize.minimize_scalar(calc_abs_residual,
                                                     bounds=(lmu, hmu),
                                                     method="bounded")
                except:
                    result = sp.optimize.minimize_scalar(calc_abs_residual,
                                                     bounds=(lmu-0.001, hmu+0.001),
                                                     method="bounded")
                mu = result.x
    return mu


#added by Yi Wang, calculate cv at constant dope
def calc_cv_x(epsilon, dos, mur, Tr, dosweight=2.):
    ...
    Returns:
        ...
        temperature and chemical potential at constant number of electrons, in SI units.
    """
    kBTr = np.array(Tr) * BOLTZMANN
    nT = len(Tr)
    nmu = len(mur)
    nruter = np.empty((nT, nmu))
    nruter_x = np.empty((nT, nmu))
    L_x = np.empty((nT, nmu))
    carrier = np.empty((nT, nmu))
    zeta = np.empty((nT, nmu))
    zeta2 = np.empty((nT, nmu))
    de = epsilon[1] - epsilon[0]
    for iT, kBT in enumerate(kBTr):
        for imu, mu in enumerate(mur):
            zeta[iT, imu] = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT) * (epsilon - mu)), epsilon)
            zeta2[iT, imu] = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT) * (epsilon - mu)**2), epsilon)
            carrier[iT, imu] = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT)), epsilon)
            nruter[iT, imu] = zeta2[iT, imu]/kBT
            nruter_x[iT, imu] = (zeta2[iT, imu]-zeta[iT, imu]*zeta[iT, imu]/carrier[iT, imu])/kBT
            carrier[iT, imu] *= Tr[iT]
            L_x[iT,imu] = nruter_x[iT, imu]/max(carrier[iT,imu],1.e-16)
            zeta[iT, imu] /= -carrier[iT, imu]

    x_ = np.empty((nT, nmu))
    for iT, kBT in enumerate(kBTr):
        for imu, mu in enumerate(mur):
            e_ = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT) * (epsilon - mu)), epsilon)
            n = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT)), epsilon)
            e_ /= n
            _e_ = -trapz((dosweight * dos * dFDde(epsilon, mu, kBT) * np.abs(epsilon - mu - e_)), epsilon)
            _e_ /= n
            x_[iT, imu] = _e_


    return BOLTZMANN_SI * nruter, BOLTZMANN_SI * nruter_x, zeta/Volt, BOLTZMANN * carrier,\
           BOLTZMANN_SI*BOLTZMANN_SI/BOLTZMANN/qe_SI/qe_SI * L_x, x_/Volt
    #print (BOLTZMANN_SI/1.6e-19*96484.)
    #return BOLTZMANN_SI * nruter, BOLTZMANN_SI *zeta/1.6e-19*96484., BOLTZMANN * carrier
...
def fermiintegrals(epsilon, dos, sigma, mur, Tr, dosweight=2., cdos=None, fermi=None):
    ...
    L0_e = np.empty((nT, nmu, 3, 3))
    L0_h = np.empty((nT, nmu, 3, 3))
    M_e = np.empty((nT, nmu, 3, 3))
    M_h = np.empty((nT, nmu, 3, 3))
    ...
    for iT, kBT in enumerate(kBTr):
        for imu, mu in enumerate(mur):
            N[iT, imu] = -trapz((dosweight * dos * FD(epsilon, mu, kBT)),epsilon)

            tmp_h = cumtrapz((dosweight * dos * (1.0-FD(epsilon, mu, kBT))),epsilon)
            _N1 = len(epsilon) - 1
            f = interpolate.interp1d(epsilon[0:_N1], tmp_h)
            if fermi is not None: Eref=fermi
            else: Eref = mu

            N_h[iT, imu] = f(Eref)
            tmp_e = cumtrapz((dosweight * dos * FD(epsilon, mu, kBT)),epsilon)
            f = interpolate.interp1d(epsilon[0:_N1], tmp_e)
            N_e[iT, imu] = tmp_e[-1] - f(Eref)

            int0 = -dosweight * dFDde(epsilon, mu, kBT)
            intn = int0 * sigma
            L0[iT, imu] = trapz(intn, epsilon, axis=2)
            """
            bf = 1/(Siemens / (Meter * Second))
            xxx = L0[iT, imu]
            print ("bbbbbbb", L0[iT, imu])
            xxx = (xxx[0,0]+xxx[1,1]+xxx[2,2])/3
            print ("bbbbbbb", 1e8/(xxx/3912*bf*2.e-14))
            """
            tmp_h = cumtrapz(intn, epsilon, axis=2)
            f = interpolate.interp1d(epsilon[0:_N1], tmp_h)
            L0_h[iT, imu] = f(Eref)
            L0_e[iT, imu] = L0[iT, imu] - L0_h[iT, imu]

            tmp_h = cumtrapz((dosweight * sigma * (1.0-FD(epsilon, mu, kBT))),epsilon)
            _N1 = len(epsilon) - 1
            f = interpolate.interp1d(epsilon[0:_N1], tmp_h)
            M_h[iT, imu] = f(Eref)/kBT
            tmp_e = cumtrapz((dosweight * sigma * FD(epsilon, mu, kBT)),epsilon)
            f = interpolate.interp1d(epsilon[0:_N1], tmp_e)
            M_e[iT, imu] = (tmp_e[:,:,-1] - f(Eref))/kBT
            #print("eeeeee", Tr[iT], M_h[iT, imu], M_e[iT, imu])

            intn *= epsilon - mu
            L1[iT, imu] = -trapz(intn, epsilon, axis=2)
            intn *= epsilon - mu
            L2[iT, imu] = trapz(intn, epsilon, axis=2)
            if cdos is not None:
                cint = int0 * cdos
                L11[iT, imu] = -trapz(cint, epsilon, axis=3)
    return N, L0, L1, L2, L11, N_h, N_e, L0_h, L0_e, M_h, M_e
...
def apply_scissor(epsilon, dos, N0, eband, desired_gap, dosweight=2.):
    ...
    for i in range(pos, dos.size):
        if dos[i] != 0.:
            ...
            break
    if -1 in (lpos, hpos):
        raise ValueError("the Fermi energie lies outside the range of " +
                         "band energies")
