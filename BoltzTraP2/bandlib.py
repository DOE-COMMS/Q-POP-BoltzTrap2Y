...
def calc_cv_x(epsilon, dos, mur, Tr, dosweight=2.):
    ...
    for iT, kBT in enumerate(kBTr):
        for imu, mu in enumerate(mur):
            zeta[iT, imu] = -(dosweight * dos * dFDde(epsilon, mu, kBT) * (epsilon - mu)).sum() * de
            zeta2[iT, imu] = -(dosweight * dos * dFDde(epsilon, mu, kBT) * (epsilon - mu)**2).sum() * de
            carrier[iT, imu] = -(dosweight * dos * dFDde(epsilon, mu, kBT)).sum() * de
            nruter[iT, imu] = (zeta2-zeta*zeta/carrier)/kBT
            carrier[iT, imu] *= Tr[iT]
            zeta[iT, imu] /= -carrier[iT, imu]
    return BOLTZMANN_SI * nruter, 27.2106 *zeta, BOLTZMANN * carrier
