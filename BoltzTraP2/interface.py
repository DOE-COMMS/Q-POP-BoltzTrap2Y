...
import BoltzTraP2.bandlibEXT
import BoltzTraP2.ioEXT
...
def parse_interpolate(args):
    ...
    info("Number of independent second-order linear response tensors:",
    ...
    info("Nvalence (after BANDANA chops off low and high lying bands) =",
         data.nelect)
    ...
    equivalences = BoltzTraP2.sphere.get_equivalences(data.atoms, data.magmom,
                                                      nktarget)
    ...
    with TimerContext() as timer:
        BoltzTraP2.serialization.save_calculation(args.output, data,
                                                  equivalences, coeffs,
                                                  metadata)
        deltat = timer.get_deltat()
...
def parse_integrate(args):
    ...
    with TimerContext() as timer:
        cv = BoltzTraP2.bandlib.calc_cv(epsilon,
                                        dos,
                                        mur,
                                        Tr,
...
def parse_dope(args):
    ...
    fermi = BoltzTraP2.bandlibEXT.solve_for_mu(epsilon,
                                            dos,
                                            data.nelect,
                                            0.,
                                            data.dosweight,
    ...
    #the following lines are added by Yi Wang, 09/24/2020
    basefn = os.path.splitext(args.bt2_file)[0]
    with open(basefn + ".dope.dos", "w") as fp:
        for i,e in enumerate(epsilon):
            fp.write('{} {}\n'.format((e-fermi)/Volt, dos[i]*Volt))

    with open(basefn + ".dope.vvdos", "w") as fp:
        for i,e in enumerate(epsilon):
            fp.write('{} {} {} {}\n'.format((e-fermi)/Volt, 
            vvdos[0,0,i]*Volt,vvdos[1,1,i]*Volt,vvdos[2,2,i]*Volt))
    
    """
    from scipy.integrate import cumtrapz, trapz, simps
    from BoltzTraP2.fd import dFDde
    int0 = -dFDde(epsilon, fermi, BOLTZMANN*50)
    intn = int0 * vvdos
    print ("aaaaaaa", trapz(intn, epsilon, axis=2))
    for iii in range(0,3):
        intn = int0 * vvdos[iii,iii,:]
        print ("bbbbbbb", trapz(intn, epsilon))
    ttdos = np.genfromtxt("/Users/yuw3/OneDrive - The Pennsylvania State University/VinCopy/LDA_AFM_b/V1.000/interpolation.dope.vvdos")  
    int0 = -dFDde(ttdos[:,0]*Volt, 0, BOLTZMANN*50)
    for iii in range(1,4):
        intn = int0 * ttdos[:,iii]/Volt
        print ("cccccccc", trapz(intn, ttdos[:,0]*Volt))
    """

    if args.NEDOS > len(epsilon):
        from BoltzTraP2.remesh import dosremesh

        from scipy.signal import savgol_filter
        if args.SMOOTH>1:
            _dos = savgol_filter(dos, args.SMOOTH, 1)
            dos = _dos
            _vvdos = np.zeros((3,3,len(epsilon)), dtype=float)
            _cdos = np.zeros((3,3,3,len(epsilon)), dtype=float)
            for i in range(3):
                for j in range(3):
                    _vvdos[i,j,:] = savgol_filter(vvdos[i,j,:], args.SMOOTH, 1)
                    for k in range(3):
                        _cdos[i,j,k,:] = savgol_filter(cdos[i,j,k,:], args.SMOOTH, 1)
            vvdos = _vvdos
            cdos = _cdos

        _dos = dosremesh(epsilon, dos, data.fermi, dos_orig=args.doscar, data=data, NEDOS=args.NEDOS, gaussian=args.NGAUSS)
        #_dos = dosremesh(epsilon, dos, fermi, dos_orig=args.doscar, data=data, NEDOS=args.NEDOS, gaussian=args.NGAUSS)
        #_dos = dosremesh(epsilon, dos, data.fermi, dos_orig=args.doscar, data=data, NEDOS=NEDOS, gaussian=NGAUSS)
        epsilon = _dos.get_emesh()
        dos = _dos.refdos(0.0, dos)
        _vvdos = np.zeros((3,3,len(epsilon)), dtype=float)
        _cdos = np.zeros((3,3,3,len(epsilon)), dtype=float)
        for i in range(3):
            for j in range(3):
                _vvdos[i,j,:] = _dos.refdos(0.0, vvdos[i,j,:])
                for k in range(3):
                    _cdos[i,j,k,:] = _dos.refdos(0.0, cdos[i,j,k,:])
        vvdos = _vvdos
        cdos = _cdos

        fermio = fermi
        try:
            print("NELECTRONS correction", _dos.NELECTRONS, data.nelect)
            data.nelect = _dos.NELECTRONS
        except:
            pass
        fermi = BoltzTraP2.bandlibEXT.solve_for_mu(epsilon,
                                                dos,
                                                ...
                                                refine=True,
                                                try_center=True)
        print ("fermi=", fermi, "fermio=", fermio, "dosweight=",data.dosweight)
        os.rename(basefn + ".dope.dos", basefn + ".dope.dos_raw")
        os.rename(basefn + ".dope.vvdos", basefn + ".dope.vvdos_raw")
        with open(basefn + ".dope.dos", "w") as fp:
            for i,e in enumerate(epsilon):
                fp.write('{} {}\n'.format((e-fermi)/Volt, dos[i]*Volt))
        with open(basefn + ".dope.vvdos", "w") as fp:
            for i,e in enumerate(epsilon):
                fp.write('{} {} {} {}\n'.format((e-fermi)/Volt, 
                vvdos[0,0,i]*Volt,vvdos[1,1,i]*Volt,vvdos[2,2,i]*Volt))
    #end changes by Yi Wang, 09/24/2020
    """
    """
    ...
    mu0 = np.empty_like(Tr)
    for iT, T in enumerate(Tr):
        mu0[iT] = BoltzTraP2.bandlibEXT.solve_for_mu(epsilon,
                                                  dos,
                                                  data.nelect,
                                                  T,
                                                  data.dosweight)
    ...
    for iT, T in enumerate(Tr):
        dopingmin = BoltzTraP2.bandlibEXT.calc_N(epsilon, dos, mumax, T,
                                              data.dosweight) + data.nelect
        dopingmin /= vuccm3
        dopingmax = BoltzTraP2.bandlibEXT.calc_N(epsilon, dos, mumin, T,
                                              data.dosweight) + data.nelect
        dopingmax /= vuccm3
        ...
        for idoping, doping in enumerate(dopingr):
            ...
            mur[iT, idoping] = BoltzTraP2.bandlibEXT.solve_for_mu(
                epsilon,
                dos,
                N,
                T,
    ...
    with TimerContext() as timer:
        ...
        for iT, T in enumerate(Tr):
            sdos[iT, :] = BoltzTraP2.bandlibEXT.smoothen_DOS_direct(
                epsilon, dos, T, mur[iT, :])
        deltat = timer.get_deltat()
        info("smoothing the DOS took {:.3g} s".format(deltat))
        """
        with open(basefn + ".dope.sdos", "w") as fp:
            for i,e in enumerate(epsilon):
                fp.write('{} {}\n'.format((e-fermi)/Volt, sdos[i]*Volt))
        """
    ...
    cv_x = np.empty((nT, ndoping))
    S_x = np.empty((nT, ndoping))
    N_x = np.empty((nT, ndoping))
    L_x = np.empty((nT, ndoping))
    N = np.empty((nT, ndoping))
    N_h = np.empty((nT, ndoping))
    N_e = np.empty((nT, ndoping))
    L0 = np.empty((nT, ndoping, 3, 3))
    L0_h = np.empty((nT, ndoping, 3, 3))
    L0_e = np.empty((nT, ndoping, 3, 3))
    M_h = np.empty((nT, ndoping, 3, 3))
    M_e = np.empty((nT, ndoping, 3, 3))
    L1 = np.empty((nT, ndoping, 3, 3))
    L2 = np.empty((nT, ndoping, 3, 3))
    Lm11 = np.empty((nT, ndoping, 3, 3, 3))
    deltaE = np.empty((nT, ndoping))
    with TimerContext() as timer:
        for iT, T in enumerate(Tr):
            cv[iT], cv_x[iT], S_x[iT], N_x[iT], L_x[iT], deltaE[iT] = BoltzTraP2.bandlibEXT.calc_cv_x(epsilon,
                                                dos,
                                                mur[iT, :],
                                                np.array([T]),
                                                dosweight=data.dosweight)
            N[iT], L0[iT], L1[iT], L2[iT], Lm11[iT], \
            N_h[iT], N_e[iT], L0_h[iT], L0_e[iT], M_h[iT], M_e[iT] \
                 = BoltzTraP2.bandlibEXT.fermiintegrals(
                 epsilon,
                 dos,
                 vvdos,
                 mur=mur[iT],
            ...
            L0_h[iT] = L0_h[iT]/ (Siemens / (Meter * Second)) / vuc
            L0_e[iT] = L0_e[iT]/ (Siemens / (Meter * Second)) / vuc
            M_h[iT] = M_h[iT]/ (Siemens / (Meter * Second)) / vuc
            M_e[iT] = M_e[iT]/ (Siemens / (Meter * Second)) / vuc
        deltat = timer.get_deltat()
        info("computing the FD moments took {:.3g} s".format(deltat))
    ...
    for iT, T in enumerate(Tr):
        (L11[iT], seebeck[iT], kappa[iT],
         hall[iT]) = BoltzTraP2.bandlibEXT.calc_Onsager_coefficients(
             L0[[iT]], L1[[iT]], L2[[iT]], mur[iT], np.array([T]), vuc,
             Lm11[[iT]])
    tracefn = basefn + ".dope.trace"
    info("trace output file:", tracefn)
    BoltzTraP2.ioEXT.save_traceEXT(tracefn,
                             data,
                             Tr,
                             mur,
                             N,
                             ...
                             L_x,
                             fermi,
                             L0_h, L0_e, M_h, M_e, N_h, N_e, deltaE,
                             scattering_model=args.scattering_model)
    ...
    if args.quantity in tensors:
        if args.components is None:
            lexit(("{} is a tensor but no components have been specified. "
                   "Use e.g. '[\"xx\",\"yy\"]' ").format(args.quantity))
    if args.quantity not in tensors:
        if components:
            warning(("{} is not a tensor. "
                     "The --components options will have no effect").format(
                         args.quantity))
        components = [()]
...
def parse_arguments():
    ...
    interpolate_parser.add_argument("-o",
                                    "--output",
    ...
    integrate_parser.add_argument("-b",
                                  "--bins",
                                  type=positive_integer,
    ...
    integrate_parser.set_defaults(scattering_model="uniform_tau",
                                  func=parse_integrate)
    integrate_parser.add_argument(
        "-s",
        "--scissor",
        type=positive_float,
    ...
    dope_parser.add_argument("bt2_file",
                             help="saved output from an interpolation step")
    ...
    dope_parser.add_argument("-p",
                             "--prefix",
                             help="prefix for the output files")
    dope_parser.add_argument("-b",
                             "--bins",
                             type=positive_integer,
                             help="number of bins for the DOS")
    dope_parser.add_argument(
        ...
        help="value of the gap, in eV,"
        " to be achieved by shifting the conduction bands")
    dope_parser.add_argument("-dos", "--doscar", dest="doscar", nargs="?", type=str, default=None,
                      help="file path to DOSCAR file to be used to renormalize the interpolated DOS's. \n"
                           "Default: None")
    dope_parser.add_argument("-nd", "--nedos", dest="NEDOS", nargs="?", type=int, default=20000,
                      help="Refined number of DOS points. \n"
                           "Default: 20000")
    dope_parser.add_argument("-sm", "--smooth", dest="SMOOTH", nargs="?", type=int, default=0,
                      help="Box size of savgol_filter to smoothen the DOS. \n"
                           "Default: 0")
    dope_parser.add_argument("-ng", "--ugauss", dest="NGAUSS", nargs="?", type=float, default=2000,
                      help="Gaussian enhancing factor for refining mesh near Fermi energy. \n"
                           "Default: 2000")
    ...
    plot_parser.add_argument("btj_file",
                             help="saved output from an integration step")
    plot_parser.add_argument("quantity",
                             choices=("cv", "n", "DOS", "sigma", "S", "kappae",
                                      "L", "PF", "RH"),
    ...
    if BoltzTraP2.fermisurface.available:
        fermisurface_parser.add_argument(
            "bt2_file", help="saved output from an interpolation step")
        fermisurface_parser.add_argument("mu",
                                         type=float,
                                         help="chemical potential [Ha]")
        fermisurface_parser.add_argument(
            "-t",
            "--thickness",
            type=float,
    ...
    #the following is changed by Yi Wang, 09/24/2020
